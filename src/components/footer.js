...
class Footer extends Component {
  sendTweet(props) {
    ...
    const tweet = `${info.homeTeamName}: ${info.homeScore} | ${
      info.awayTeamName
    }: ${info.awayScore}`;
    ...
    $.ajax({
      type: "POST",
      url:
        "http://code.randomlysa.com/tweetScores/php/tweet.php?clientid=" +
        clientid,
      data: { newTweet: tweet }
    })
      .then(function(data) {
    ...
    if (this.props.teamsAndScores.twitterAuth) {
      return (
        <div className="footer div-center">
          ...
          <button onClick={this.sendTweet.bind(this)}>Tweet Score</button>
          <br />
          <button onClick={clearStorage}>Delete all data</button>
          <button onClick={this.props.actions.unauthTwitter}>
            remove twitter authorization
          </button>
